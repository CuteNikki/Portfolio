...
export default function NewPostPage() {
  ...
  // Track which action the user is intending to take
  ...
  const handleSubmit = (event: React.FormEvent<HTMLFormElement>) => {
    ...
    // Append the boolean directly based on the button they clicked
    formData.append(
      'isPublished',
    ...
    startTransition(async () => {
      await createPost(formData)
        .then((post) => {
          ...
          window.location.href = LINKS.dashboardPostEditWithId(post.id).url;
        })
        .catch((error) => {
